...
- Please note that this assumes you are able to make your own interferograms. This software is tested with interferograms produced for PALSAR using the ROIPAC or ISCE (up to v 2.2: insarApp.py workflow; 2.3+: stripmapApp.py workflow) software - hence we provide three test_example folders ('test_example_ISCE_insarApp', 'test_example_ISCE_stripmapApp', 'test_example_ROIPAC'). Each folder contains a text file starting with 'NOTES_' describing how to obtain the example interferograms to be used to test this software installation. ***We recommend that the ISCE stripmapApp.py workflow is used, as the other options have been deprecated and are not actively supported***.
